...
import { isMoment } from "_moment@2.24.0@moment";
...
class Lists extends React.Component {
	...
	render() {
		const { hotData, newData, recommendData, selectedTab } = this.props,
			newGroup = hotData.toJS(),
			newnewData = newData.toJS(),
			newrecommendData = recommendData.toJS();
		return (
			<React.Fragment>
				{this.renderNullData(newGroup, newnewData, newrecommendData, selectedTab)}
				{newnewData.length < 0 || newrecommendData.length < 0 || newGroup.length < 0 && <Loading />}
